        ...
        <div class="card-body">
            <form action="{{route('students.update', $student->id)}}" class="" method="post" enctype="multipart/form-data">
                ...
                <div class="mb-3">
                    ...
                    @enderror" name="name" placeholder="Tulis Nama" value="{{old('name') ?? $student->name}}">
                    @error('name')
                ...
                <div class="mb-3">
                    ...
                    @enderror" name="address" placeholder="Tulis Alamat" value="{{old('address') ?? $student->address}}">
                    @error('address')
                    <span class="invalid-feedback">{{$message}}</span>
                    @enderror
                </div>
                <div class="mb-3">
                    ...
                    @enderror" name="phone_number" placeholder="Tulis Nomor Telephone" value="{{old('phone_number') ?? $student->phone_number}}">
                    @error('phone_number')
                    <span class="invalid-feedback">{{$message}}</span>
                    @enderror
                </div> 
                ...
                <div class="mb-3">
                    ...
                    @enderror" placeholder="Masukan Foto" value="{{old('photo'?? $student->photo)}}">
                    @error('photo')
                        <span class="invalid-feedback">{{$message}}</span>
